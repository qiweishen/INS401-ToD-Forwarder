...
EthernetSocket::EthernetSocket(std::string interface_name, const MacAddress &target_mac, const std::size_t recv_buffer_size,
							   const bool enable_bpf) :
	interface_name_(std::move(interface_name)), target_mac_(target_mac), recv_buffer_size_(recv_buffer_size), enable_bpf_(enable_bpf) {
	CreateSocket();
	if (socket_fd_ < 0) {
...
namespace Ethernet {
	...
	bool SetupBpfFilter(const MacAddress target_mac, const MacAddress local_mac, const int socket_fd) {
		...
		sock_filter filter[] = {
			BPF_STMT(BPF_LD | BPF_W | BPF_ABS, 6),	BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, target_mac_hi, 0, 6),
			BPF_STMT(BPF_LD | BPF_H | BPF_ABS, 10), BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, target_mac_lo, 0, 4),
			BPF_STMT(BPF_LD | BPF_W | BPF_ABS, 0),	BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, local_mac_hi, 0, 2),
			BPF_STMT(BPF_LD | BPF_H | BPF_ABS, 4),	BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, local_mac_lo, 8, 0),
			BPF_STMT(BPF_LD | BPF_W | BPF_ABS, 6),	BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, local_mac_hi, 0, 5),
			BPF_STMT(BPF_LD | BPF_H | BPF_ABS, 10), BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, local_mac_lo, 0, 3),
			BPF_STMT(BPF_LD | BPF_W | BPF_ABS, 0),	BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, target_mac_hi, 0, 1),
			BPF_STMT(BPF_LD | BPF_H | BPF_ABS, 4),	BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, target_mac_lo, 0, 1),
	...
	std::string ParseMacAddress(const uint8_t *mac_ptr) {
		...
		char buf[18];
		std::snprintf(buf, sizeof(buf), "%02x:%02x:%02x:%02x:%02x:%02x", mac_ptr[0], mac_ptr[1], mac_ptr[2], mac_ptr[3], mac_ptr[4],
					  mac_ptr[5]);
		return buf;
	}
