...
std::map<std::string, DeviceInfo> INSDeviceDiscover::DiscoverDevices(int discovery_time_ms) {
    ...
    for (const auto &interface : interfaces) {
        threads.emplace_back([this, interface, discovery_time_ms]() {
...
bool INSDeviceDiscover::ParseResponse(const std::string &interface, const MacAddress &local_mac, const uint8_t *buffer,
    ...
    if (std::memcmp(buffer, broadcast_mac_.data(), kMacAddressSize) == 0) return false;

    if (buffer[kEthernetHeaderSize] != COMMAND_START_BYTES[0] ||
        buffer[kEthernetHeaderSize + 1] != COMMAND_START_BYTES[1]) {
        return false;
    }
