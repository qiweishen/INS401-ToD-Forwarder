/// @file rs232_sender.h
/// @brief Low-latency RS-232 serial sender for NMEA $GPZDA Time-of-Day messages.
...
class RS232Sender {
public:
    RS232Sender(const ForwarderConfig &config);
    ...
    bool SendGNZDA(std::uint16_t gps_week, std::uint32_t gps_millisecs);
...
private:
    ...
    static void GpsTimeToUtc(std::uint16_t gps_week, std::uint32_t gps_ms, int leap_seconds,
